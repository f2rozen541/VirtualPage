...
/**
	 * {@code HuBar} instance used to control the HuBar.
	 */
	private HuBar mHuBar = null;
    ...
    private String TAG = "SaturationBar";

    /**
     * set the saturatino of the given color
     */
    public void setSaturationValue(float value) {
        mBarPointerPosition = (int) (value * mBarLength + mBarPointerHaloRadius);
...
//		mBarPointerHaloPaint.setAlpha(0x50);
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (!isInEditMode()){
			...
			shader = new LinearGradient(mBarPointerHaloRadius, 0,
					x1, y1, new int[] {
							Color.WHITE, Color.RED }, null, Shader.TileMode.CLAMP);
...
//		canvas.drawCircle(cX, cY, mBarPointerRadius, mBarPointerPaint);
		...
		case MotionEvent.ACTION_MOVE:
			if (mIsMovingPointer) {
				...
				if (dimen >= mBarPointerHaloRadius
					...
					if (mHuBar != null) {
                        mHuBar.setColor(mColor);
                        mHuBar.changeValueBarColor(mColor);
                        mHuBar.changeOpacityBarColor(mColor);
					}
					invalidate();
	...
	public void setColor(int color) {
		...
		if (mHuBar != null) {
            mHuBar.setColor(mColor);
			if(mHuBar.hasValueBar())
                mHuBar.changeValueBarColor(mColor);
			else if(mHuBar.hasOpacityBar())
                mHuBar.changeOpacityBarColor(mColor);
		}
		invalidate();
	}
	...
	public void setSaturation(float saturation) {
		mBarPointerPosition = (int) (((mSatToPosFactor * saturation))
                        + mBarPointerHaloRadius);
		calculateColor(mBarPointerPosition);
		mBarPointerPaint.setColor(mColor);
		if (mPicker != null) {
         ...
         */
	private void calculateColor(float coord) {
	    ...
	    if (coord <= 0) {
            ...
            mcurrentValue = (Math.round(mPosToSatFactor * coord * 100));
        }
        Log.d(TAG, "calculateColor: " + Math.round(mPosToSatFactor * coord * 100) + "\tcoord:" + coord + "\tmPosToSatFactor:" + mPosToSatFactor);
	 ...
	 * @param huBar
	 */
	public void setHuBar(HuBar huBar) {
		mHuBar = huBar;
	}
